...
export default async function TripDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="publicPage"><div className="publicPageInner">
    ...
    <div className="publicDetailSections">
      ...
      <section><h2>Jadwal & kursi</h2>{trip.departures.length?trip.departures.map((departure:any)=><div className="publicSchedule" key={departure.id}><span>{new Date(departure.startsAt).toLocaleDateString('id-ID',{dateStyle:'long'})}<small>{departure.meetingPoint}</small></span><b>{Math.max(0,departure.maxPax-departure.reservedPax)} kursi</b>{Number(departure.surchargeAmount)>0&&<small>{departure.surchargeLabel}: {money(departure.surchargeAmount)} / peserta</small>}</div>):<p>Tanggal fleksibel dan dikonfirmasi bersama tim.</p>}</section>
