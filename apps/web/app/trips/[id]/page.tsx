...
export default async function TripDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="publicPage"><div className="publicPageInner">
    <section className="publicDetailHero"><div><span>{trip.departures?.length?'OPEN TRIP':'PRIVATE TRIP'}</span><h1>{trip.name}</h1><p>{trip.publicDescription}</p><PublicBookingForm trip={trip}/></div><aside className="publicDetailPrice"><small>Mulai dari</small><strong>{money(adult?.sellingPrice||0)}</strong><span>dewasa · {trip.durationDays} hari</span></aside></section>
