...
import { WhatsAppCustomerIntake } from './whatsapp-customer-intake';
...
const navItems = [
  ...
  { href: '/content', label: 'Artikel & Promo', icon: 'C' },
  { href: '/media-library', label: 'Media Website', icon: 'M' },
  { href: '/asset-knowledge', label: 'Asset & Knowledge', icon: 'AK' },
  { href: '/archives', label: 'Arsip File', icon: 'A' },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--brand-canvas)', color: 'var(--brand-ink)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>{pathname==='/crm/customers'&&<WhatsAppCustomerIntake/>}{pathname==='/operations/trips'&&<TripBookingConfirmation/>}{pathname?.startsWith('/packages')&&<><PackageRequirementsPanel/><PackageAddonsPanel/></>}{children}</div>
    </div>
  );
}
