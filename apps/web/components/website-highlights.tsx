...
export function WebsiteHighlights({
  onSelect,
}: {
  ...
  const openTrips = useMemo(
    () => packs.filter((p) => p.departures?.length).slice(0, 4),
    [packs],
  );
  ...
  return (
    <>
      ...
      <section className="dealZone" id="flash-sale">
        <div className="flashPanel">
          <div>
            ...
            <p>
              {flash?.description ??
                "Promo khusus untuk paket pilihan. Hubungi tim kami untuk penawaran terbaik."}
            </p>
            {flash ? (
              <>
                <strong>
                  {flash.discountType === "PERCENTAGE"
                    ? `${flash.discountValue}% OFF`
                    : `Potongan ${money(flash.discountValue)}`}
                </strong>
                <small>
                  Kode: {flash.code} · berakhir{" "}
                  {new Date(flash.endsAt).toLocaleDateString("id-ID")}
                </small>
              </>
            ) : (
              <strong>Harga spesial segera hadir</strong>
            )}
          </div>
          ...
          {openTrips.length ? (
            openTrips.map((p) => {
              ...
              return (
                <button
                  type="button"
                  className={`scheduleCard ${premium ? "premium" : "regular"}`}
                  key={p.id}
                  onClick={() => select(p.id)}
                >
                  <div className="scheduleThumb">
                    ...
                    <time>
                      <strong>
                        {new Date(d.startsAt).toLocaleDateString("id-ID", {
                          day: "2-digit",
                        })}
                      </strong>
                      <span>
                  ...
                  <div className="scheduleInfo">
                    <small>
                      {p.durationDays} hari · {p.destination}
                    </small>
                    <h3>{p.name}</h3>
                    <div>
                      ...
                      <span>
                        🕒{" "}
                        {new Date(d.startsAt).toLocaleTimeString("id-ID", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}{" "}
                        WIB
                    ...
                    <footer>
                      <strong>{money(p.prices[0]?.sellingPrice)}</strong>
                      <em>Lihat detail →</em>
                    </footer>
                  </div>
                </button>
            ...
            <article className="emptyCompact">
              <div>
                <h3>Jadwal baru segera dibuka</h3>
                <p>Pantau pengumuman Open Trip berikutnya.</p>
              </div>
            </article>
          )}
        </div>
      ...
      <section className="highlightSection" id="popular">
        <header>
          <div>
            <span className="dealEyebrow">PALING DICARI</span>
            <h2>Trip populer</h2>
          </div>
        </header>
        <div className="popularTiles">
          {popular.map((p, n) => (
            <button type="button" onClick={() => select(p.id)} key={p.id}>
              <i>{["🌆", "🏝️", "🛳️", "🌏"][n]}</i>
              <span>
                <small>
                  {p.durationDays} hari · {p.destination}
              ...
              <em>{money(p.prices[0]?.sellingPrice)}</em>
            </button>
          ))}
        </div>
      </section>
