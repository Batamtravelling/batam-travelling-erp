...
@Injectable() class TransactionsService {
  ...
  async createPayment(i:RequestIdentity,d:CreatePaymentDto){return this.prisma.$transaction(async tx=>{const invoice=await tx.invoice.findFirst({where:{id:d.invoiceId,tenantId:i.tenantId}});if(!invoice)throw new NotFoundException('Invoice tidak ditemukan');if(d.amount>Number(invoice.totalAmount)-Number(invoice.paidAmount))throw new BadRequestException('Pembayaran melebihi outstanding invoice');const paymentNumber=await this.codes.nextPayment(tx,i.tenantId);return tx.payment.create({data:{tenantId:i.tenantId,paymentNumber,invoiceId:invoice.id,customerId:invoice.customerId,amount:d.amount,method:d.method,reference:d.reference,notes:d.notes,receivedById:i.userId}})})}
