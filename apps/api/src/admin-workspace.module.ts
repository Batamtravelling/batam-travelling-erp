import { BadRequestException, Body, Controller, ForbiddenException, Get, Inject, Injectable, Module, Param, Patch, Post, Query, Req, UseGuards } from '@nestjs/common';
...
export class AdminWorkspaceService {
  ...
  async dashboard(i: RequestIdentity) {
    const [leads, customers, bookings, invoices, tasks, trips] = await Promise.all([
      ...
      i.permissions.has('task.read') ? this.p.task.findMany({ where: { tenantId: i.tenantId, OR: [{ assigneeId: i.userId }, { participants: { some: { userId: i.userId } } }], status: { notIn: ['DONE','CANCELLED'] } }, include: { project: { select: { name: true } }, trip: { select: { title: true, tripCode: true } } }, orderBy: { dueDate: 'asc' }, take: 8 }) : Promise.resolve([]),
      i.permissions.has('trip.read') ? this.p.trip.findMany({ where: { tenantId: i.tenantId, assignments: { some: { employeeId: i.userId } }, status: { notIn: ['COMPLETED','CANCELLED'] } }, orderBy: { startsAt: 'asc' }, take: 8 }) : Promise.resolve([]),
    ]);
  ...
  async reviewPromotion(i:RequestIdentity,id:string,d:PromotionReviewDto){const current=await this.p.promotion.findFirst({where:{id,tenantId:i.tenantId},include:{packages:true}});if(!current)throw new BadRequestException('Promo tidak ditemukan');if(['SUBMIT','ARCHIVE'].includes(d.action)&&!i.permissions.has('content.manage'))throw new ForbiddenException('Permission content.manage diperlukan');if(['APPROVE','REJECT'].includes(d.action)&&!i.permissions.has('content.approve'))throw new ForbiddenException('Permission content.approve diperlukan');if(d.action==='SUBMIT'){if(!['DRAFT','REJECTED'].includes(current.approvalStatus))throw new BadRequestException('Promo tidak dapat diajukan dari status saat ini');await this.validatePromotionPackages(i,current.packages.map(x=>x.packageId),true)}else if(d.action==='ARCHIVE'){if(current.status==='ARCHIVED')throw new BadRequestException('Promo sudah diarsipkan')}else{if(current.approvalStatus!=='PENDING_REVIEW')throw new BadRequestException('Promo belum menunggu review');if(current.submittedById===i.userId)throw new BadRequestException('Pengaju promo tidak boleh menyetujui atau menolak pengajuannya sendiri');if(d.action==='REJECT'&&!d.note?.trim())throw new BadRequestException('Catatan wajib untuk penolakan');if(d.action==='APPROVE')await this.validatePromotionPackages(i,current.packages.map(x=>x.packageId),true)}return this.p.$transaction(async tx=>{const note=d.note?.trim();const data=d.action==='SUBMIT'?{status:'DRAFT' as const,approvalStatus:'PENDING_REVIEW' as const,submittedById:i.userId,submittedAt:new Date(),reviewedById:null,reviewedAt:null,reviewNote:null}:d.action==='APPROVE'?{status:'PUBLISHED' as const,approvalStatus:'APPROVED' as const,reviewedById:i.userId,reviewedAt:new Date(),reviewNote:note}:d.action==='REJECT'?{status:'DRAFT' as const,approvalStatus:'REJECTED' as const,reviewedById:i.userId,reviewedAt:new Date(),reviewNote:note}:{status:'ARCHIVED' as const};const row=await tx.promotion.update({where:{id},data});await tx.auditLog.create({data:{tenantId:i.tenantId,actorId:i.userId,action:`promotion.${d.action.toLowerCase()}`,resourceType:'Promotion',resourceId:id,requestId:i.requestId,metadata:{fromStatus:current.status,fromApprovalStatus:current.approvalStatus,toStatus:row.status,toApprovalStatus:row.approvalStatus,note:note??null}}});return row})}
...
@UseGuards(IdentityGuard,PermissionGuard) @Controller() class AdminWorkspaceController{
 ...
 @Patch('package-reviews/:id') @Permissions('package.read') review(@CurrentIdentity()i:RequestIdentity,@Param('id')id:string,@Body()d:PackageReviewDto){return this.s.reviewPackage(i,id,d)}
 ...
 @Patch('content/promotions/:id/review') @Permissions('content.read') reviewPromotion(@CurrentIdentity()i:RequestIdentity,@Param('id')id:string,@Body()d:PromotionReviewDto){return this.s.reviewPromotion(i,id,d)}
